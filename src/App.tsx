...
function Footer({ version }: { version: string }) {
  ...
  return (
    <footer className="mt-6 mb-2 flex flex-col items-center gap-2">
      <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-xs font-medium leading-none">
        <button
          onClick={() => { openUrl("https://github.com/XueshiQiao/HyperCapslock").catch(console.error); }}
          title={t("footer.github")}
          aria-label={t("footer.github")}
          className="flex items-center text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors"
        >
          <svg className="w-3.5 h-3.5 block" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        ...
        <button
          onClick={() => { openUrl("https://x.com/XueshiQiao").catch(console.error); }}
          title="@XueshiQiao on X"
          aria-label="@XueshiQiao on X"
          className="flex items-center gap-1 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors"
        >
          <svg className="w-3 h-3 block" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
          </svg>
          <span className="leading-none">@XueshiQiao</span>
        </button>
      </div>
      <div className="flex items-center gap-1.5">
        <span className="text-[11px] text-slate-400 dark:text-slate-500">
          {t("footer.more_apps_desc")}
        </span>
        <button
          onClick={() => { openUrl("https://xueshi.dev").catch(console.error); }}
          className="text-[11px] text-blue-500/80 hover:text-blue-500 dark:hover:text-blue-400 hover:underline transition-colors"
        >
          xueshi.dev
        </button>
      </div>
